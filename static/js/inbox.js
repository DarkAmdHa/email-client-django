...
function load_mailbox(mailbox) {
  ...
  if (mailbox === 'inbox') {
    ...
    <input type="text" class="form-control searchInput" placeholder="Search" aria-label="Search" aria-describedby="basic-addon1">
    </div>`
    document.querySelector('#emails-view').innerHTML += searchBar

    document
      .querySelector('#emails-view .searchInput')
      .addEventListener('keypress', (e) => {
        if (e.keyCode === 13) {
          fetch(`/emails/search/${e.target.value}`)
            .then((response) => response.json())
            .then((emails) => {
              if (emails.length > 0) {
                document
                  .querySelectorAll('#emails-view .card.my-1.items')
                    ...
                    if (email.body.length <= 99) {
                      div.innerHTML = `
                                          <div class='card ${is_read}'>
                                              ...
                                              <div class='card-body ${is_read}' id='item-${
                        email.id
                      }'>
                                                  <p class='card-title'>
                                                      <strong>From:</strong> <strong>From:</strong> <strong><span class='text-info'>${
                                                        email.sender
                                                      }</span></strong> &nbsp; |  &nbsp; 
                                                      <strong>To:</strong> <strong>From:</strong> <strong><span class='text-info'>${
                                                        email.recipients
                                                      }</span></strong> &nbsp; |  &nbsp;  
                                                      <strong>Date:</strong> ${
                                                        email.timestamp
                      ...
                      div.innerHTML = `
                        ...
                        email.id
                                                  ...
                                                  <p class='card-title'>
                                                      <strong>From:</strong> <strong><span class='text-info'>${
                                                        email.sender
                                                      }</span></strong> &nbsp; |  &nbsp;
                                                      <strong>To:</strong> <strong><span class='text-info'>${
                                                        email.recipients
                                                      }</span></strong> &nbsp; |  &nbsp;
                                                      <strong>Date:</strong> ${
                                                        email.timestamp
  ...
  fetch(`/emails/${email_id}`)
    ...
    .then((email) => {
      ...
      archiveBtn.className = `btn ${
        email.archived ? 'btn-warning' : 'btn-danger'
      } my-2`

      archiveBtn.addEventListener('click', () => {
        archive_and_unarchive(email_id, email.archived)

        if (archiveBtn.innerText == 'Delete') {
          archiveBtn.innerText = 'Restore'
        } else {
      ...
      if (!email.archived) {
        archiveBtn.innerHTML = `<i class='fas fa-trash'></i> Delete`
      } else {
        archiveBtn.innerHTML = `<i class='fas fa-folder'></i> Restore`
      }
